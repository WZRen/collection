...
import com.cyril.collection.widget.calendar.BaseCalendarListAdapter;
...
public class DayNewsListAdapter extends BaseCalendarListAdapter<StoriesBean> {
    ...
    public View getSectionHeaderView(String date, View convertView, ViewGroup parent) {
        ...
        List<StoriesBean> modelList = dateDataMap.get(date);
        if (convertView != null) {
        ...
        headerViewHolder.yearMonthText.setText(CalendarListActivity.YEAR_MONTH_FORMAT.format(calendar.getTime()));
        return convertView;
    }

    @Override
    public View getItemView(StoriesBean model, String month, int pos, View convertView, ViewGroup parent) {
        ContentViewHolder contentViewHolder;
        if (convertView != null) {
            contentViewHolder = (ContentViewHolder) convertView.getTag();
        } else {
            convertView = inflater.inflate(R.layout.calendar_content, null);
        ...
        contentViewHolder.timeTextView.setText(month);

        Picasso.with(convertView.getContext()).load(Uri.parse(model.getImages().get(0))).into(contentViewHolder.newsImageView);
        return convertView;
    }
